...
class YoloDataPipelineApp:
    def __init__(self, root):
        ...
        self.root.configure(bg="#F4F6F9") # Modern light-grey background
        ...
        # --- Top Header Section (Project Selection) ---
        self.setup_header()
        
        # --- Main Navigation Tab (Notebook) ---
    ...
    def setup_styles(self):
        ...
        # LabelFrame (Group Boxes) styling
...
{folder}")
    ...
    def setup_tab_grab(self):
        ...
        tk.Label(info_box, text="PETUNJUK TOMBOL TOMBOL JENDELA KAMERA:
...
• Tekan [ Q ] pada keyboard untuk menghentikan dan menutup aliran kamera.", 
                 font=("Segoe UI", 9, "italic"), fg="#2B6CB0", bg="#EBF8FF", justify="left", padx=15, pady=10).pack(anchor="w")
...
Tekan 'Q' untuk Selesai.")
    ...
    def setup_tab_aug(self):
        ...
        tk.Label(warn_box, text="PENTING: Lakukan anotasi di X-AnyLabeling terlebih dahulu, lalu simpan hasil ekspor
berformat 'YOLO OBB' ke dalam folder '2_Exported_AnyLabeling' sebelum menekan tombol.", 
                 font=("Segoe UI", 9), fg="#C53030", bg="#FFF5F5", justify="left", padx=12, pady=8).pack(anchor="w")

        # Checkboxes inside a neat LabelFrame
...
{berhasil} data asli berhasil diekspansi menjadi total {berhasil * (num_aug + 1)} file citra teraugmentasi di folder '3_Augmented'.")
    ...
    def setup_tab_split(self):
        ...
        tk.Label(info_yaml, text="Catatan Sistem:
Aplikasi akan membaca daftar objek dari berkas 'classes.txt' di folder proyek secara otomatis untuk mengompilasi susunan file konfigurasi dataset.yaml.", 
                 font=("Segoe UI", 9), fg="#4A5568", bg="#F7FAFC", justify="left", padx=15, pady=12).pack(anchor="w")
    ...
    def run_split(self):
        ...
        try:
            ...
            messagebox.showerror("Error Angka", "Nilai input rasio pembagian harus berupa angka integer murni!")
            return
...
Struktur folder '4_YOLO_Ready' beserta skrip 'dataset.yaml' siap dipasok ke mesin training.")
    ...
    def setup_tab_train(self):
        ...
        # --- Visual Real-Time Training Progress Indicator Box ---
    ...
    def _run_train(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self.lbl_train_status.config(text="Status Workspace: Sukses Total! Bobot Model Tersimpan Sekat 5.", fg="#38A169"))
