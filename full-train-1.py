...
class YoloDataPipelineApp:
    ...
    def set_project_dir(self):
        ...
        if folder:
            ...
            # Menambah folder 5_Training_Results secara otomatis
    ...
    def _run_train(self):
        ...
        # MENENTUKAN LOKASI PENYIMPANAN HASIL TRAINING
        ...
        try:
            ...
            # Eksekusi training dengan tambahan argumen project dan name
            model.train(
                data=yaml_path, 
                epochs=epochs, 
                imgsz=imgsz, 
                ...
                patience=patience, 
                workers=workers,
                project=target_dir, # <-- Mengarahkan YOLO ke folder project utama
                name="Sesi_Training" # <-- Nama sub-folder hasilnya
            )
            
            self.root.after(0, lambda: self.lbl_train_status.config(text="Status: Training Selesai! Cek folder '5_Training_Results'."))
            self.root.after(0, lambda: messagebox.showinfo("Selesai", "Proses Training telah berhasil diselesaikan!\nFile best.pt kamu tersimpan di folder 5_Training_Results."))
